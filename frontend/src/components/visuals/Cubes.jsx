...
const Cubes = ({
  ...
  const onClick = useCallback(
    e => {
      ...
      Object.keys(rings).map(Number).sort((a, b) => a - b).forEach(ring => {
        ...
        gsap.to(faces, { backgroundColor: rippleColor, duration: 0.3 / rippleSpeed, delay, ease: 'power3.out' });
        gsap.to(faces, { backgroundColor: faceColor, duration: 0.3 / rippleSpeed, delay: delay + 0.45 / rippleSpeed, ease: 'power3.out' });
      });
    },
  ...
  useEffect(() => {
    ...
    const loop = () => {
      if (!userActiveRef.current) {
        ...
        tiltAt(pos.y, pos.x);
        if (Math.hypot(pos.x - tgt.x, pos.y - tgt.y) < 0.1) simTargetRef.current = { x: Math.random() * gridSize, y: Math.random() * gridSize };
      }
  ...
  const wrapperStyle = {
    ...
    '--cube-face-shadow': shadow === true ? '0 0 6px rgba(53,37,205,.22)' : shadow || 'none',
